...
const (
	...
	LogReportDuplicateSubmission      = "report duplicate submission: report_id=%d reporter=%d product_id=%d"
	LogReportHandleSuccess            = "report handled: report_id=%d action=%s"
	LogReportHandleFailed             = "report handle failed: report_id=%d admin=%d error=%v"
	LogReportHandleRejected           = "report handle rejected: report_id=%d admin=%d reason=%s"
)

// LogTemplateCount guards the "at least 25 templates" requirement.
const LogTemplateCount = 34
