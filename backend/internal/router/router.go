...
func New(cfg *config.Config, db *gorm.DB, logger *slog.Logger) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		RegisterReportRoutes(v1, reportH, auth, apiLimiter)
		RegisterAdminReportRoutes(v1, reportH, auth, requireAdmin, apiLimiter)
		// admin-only stats route group (kept for RBAC coverage)
